...
from properties_MIT import get_scoring_function, qed_func, sa_func
...
def train_agent(restore_prior_from='Prior.ckpt',
    ...
    if torch.cuda.is_available():
        ...
        Prior.rnn.load_state_dict(torch.load('Prior.ckpt', map_location=lambda storage, loc: storage))
    ...
    for step in range(n_steps):
        ...
        if step%10==0 and step!=0:
            torch.save(Agent.rnn.state_dict(), './model/'+str(step)+'.ckpt')
